...
export default {
  ...
  methods: {
    watchCourse(status,lessonid,index,media) { // 播放视频（章节，每节课的索引，每节课的视频地址）
    if(media == null || media == ""){
...
// 锁上的，先验证是否登录
        if(!this.isLogin){
          ...
          // 登录后，再验证是否购买
          if(!this.isBuy){  // 判断买没买
            this.$message.warning("请先购买并解锁，才能观看本视频！")
          }else{
            this.$message.success("观看第【" + lessonid + "】节课程视频！");
            this.$router.push({ name: "videoDetail", params: { course: this.course , lessonid:lessonid,isBuy:this.isBuy } });
          }
    ...
    createCode(){
      // 去获取支付连接
      this.axios.get("http://localhost:80/order/createCode",{
        params:{
          courseId:this.course.id,
          courseName:this.course.courseName,
          price:1 //测试支付金额固定为1分钱，真实上线环境再改 真实价钱 this.course.discounts,
        }
      }).then((res) => {
        console.log(res);
        ...
        width:200,
        height:200,
        text:res.data.code_url  // 将返回的数据嵌入到二维码中
      });
...
// 真实的修改数据库，保存订单！
    ...
    updateOrder(statusCode){
      return this.axios.get("http://localhost:80/order/updateOrder",{
        params:{
          orderNo:this.orderNo,
          status:statusCode,
        }
    ...
    // 获取本课程的全部留言
    getComment(){
      return this.axios.get("http://localhost:80/course/comment/getCourseCommentList/"+this.course.id+"/1/20").then((res) => {
        console.log(res);
    ...
    getMyCourseList(){  // 查询当前用户购买过的全部课程
    return this.axios.get("http://localhost:80/course/getCourseByUserId/"+this.user.content.id).then((res) => {
        console.log(res);
  ...
  // 发表留言
  saveComment(){
    return this.axios.get("http://localhost:80/course/comment/saveCourseComment",{
      params:{
        courseId:this.course.id,
    ...
    // 点赞
    zan(comment){     
      return this.axios.get("http://localhost:80/course/comment/saveFavorite/"+ comment.id +"/"+this.user.content.id).then((res) => {
       // 重新获取本门课的全部留言信息
       this.getComment();
      }).catch((err) => {
    ...
    // 取消赞
    cancelzan(comment){
      return this.axios.get("http://localhost:80/course/comment/cancelFavorite/"+ comment.id +"/"+this.user.content.id).then((res) => {
       // 重新获取本门课的全部留言信息
       this.getComment();
      }).catch((err) => {
