...
export default {
  ...
  methods: {
    //获取资源分类信息
    getCateList() {
      this.listLoading = true;  // 开启加载动画
      return axios.get("/ResourceCategory/findAllResourceCategory").then(res => {
    ...
    //添加&修改资源分类
    handleSave() {
      return axios.post("/ResourceCategory/saveOrUpdateResourceCategory",this.resourceCategory).then(res => {
        this.dialogVisible = false; // 关闭对话框
        this.getCateList(); // 获取资源分类信息
      }).catch()
    },

    //删除
    handleDelete(index, row) {
      return axios.get("/ResourceCategory/deleteResourceCategory?id=" + row.id).then(res => {
        this.getCateList(); // 获取资源分类信息
      }).catch()
    }
  }
