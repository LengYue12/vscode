...
export default {
  ...
  methods: {
    ...
    getResourceList() {
      ...
      return axios.post("/resource/findAllResource",this.listQuery).then(res => {
    ...
    //添加&修改资源
    handleSave() {
      return axios.post("/resource/saveOrUpdateResource",this.resource).
      then(res => {
    ...
    //删除资源
    handleDelete(row) {
      return axios.get("/resource/deleteResource?id="+ row.id).then(res => {
        this.getResourceList();
      }).catch()
    },
